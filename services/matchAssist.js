// ─────────────────────────────────────────────────────────────────────────────
// CampusTrace — Match scoring (Backend authority)
//
// This is ONE of the ONLY TWO modules in the whole project allowed to contain
// the match scoring formula and threshold logic. The frontend twin lives in
// client/src/utils/matchAssist.js and MUST stay byte-for-byte equivalent in
// behaviour (same inputs → same { score, reasons }).
//
// No Controller / Route / component may re-implement the formula, the threshold
// comparison, or the pairKey format. Always call the helpers exported here.
// ─────────────────────────────────────────────────────────────────────────────

// Match rules v2 — the single place these numbers are defined on the backend.
const MATCH_RULES = {
 ...
 * @param {object} [options] - optional overrides for MATCH_RULES (used by tests)
...
const meetsDisplayThreshold = (score, options = {}) => {
  const rules = { ...MATCH_RULES, ...options };
...
const meetsConflictThreshold = (score, options = {}) => {
  const rules = { ...MATCH_RULES, ...options };
